#ifndef __LINMOD2_DISTRIBUTIONS_NORMAL__
#define __LINMOD2_DISTRIBUTIONS_NORMAL__
...
static inline double dnorm(const double x, const double mean, const double sd, const bool log_)
{
  double ret;
  const double tmp = x - mean;
  
  ret = LINMOD2_ROOT2INV*LINMOD2_SQPIINV/sd*exp(-tmp*tmp*0.5/(sd*sd));
...
static inline double pnorm(const double q, const double mean, const double sd, const bool lower_tail, const bool log_p)
{
  double ret;
  
  ret = 0.5*erfc(-LINMOD2_ROOT2INV*(q - mean)/sd);
...
static inline double qnorm(const double p, const double mean, const double sd, const bool lower_tail, const bool log_p)
{
